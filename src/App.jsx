...
const ISRAEL_PAYMENT_URL_HE = "https://www.jgive.com/new/he/ils/donation-targets/172261";
const ISRAEL_PAYMENT_URL_EN = "https://www.jgive.com/new/en/usd/donation-targets/172261";
...
const data = {
  he: {
    ...
    storyCta: "הסיפור של השומרון",
    backToLanding: "חזרה לדף הראשי",
    ...
    popular: "מומלץ",
    monthlyCta: "לחץ לבחור רמת שותפות",
  ...
  en: {
    ...
    popular: "Recommended",
    monthlyCta: "Click to choose a partnership level",
...
<div className="absolute inset-0 bg-gradient-to-t from-[#0B1320]/75 via-transparent to-[#0B1320]/10" />
  ...
  <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm text-[#E9D9A8] backdrop-blur">
    ...
    {t.badge}
      ...
      {/* Why this is different moved into the hero card */}

      {/* Why now section hidden for now */}
      ...
      <section className="bg-[#FBF8F1] px-6 py-18 md:px-10 md:py-20">
        <div className="mx-auto max-w-7xl">
          <Heading kicker={t.activityKicker} title={t.activityTitle} text={t.activityText} />
          <div className="mt-10 grid gap-5 md:grid-cols-4">
            {activities[language].map(([Icon, title, text]) => (
              <div key={title} className="rounded-3xl border border-[#E4D8BC] bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#E9D9A8]/70 text-[#344D32]">
                  <Icon className="h-6 w-6" />
                </div>
                <h3 className="mt-5 text-xl font-bold text-[#0B1320]">{title}</h3>
      ...
      <section className="relative overflow-hidden px-6 py-18 md:px-10 md:py-20">
        ...
        />
        <div className="absolute inset-0 bg-[#FBF8F1]/55" />
        <div className="absolute inset-0 bg-gradient-to-b from-[#FBF8F1]/75 via-[#FBF8F1]/35 to-[#FBF8F1]/80" />
        <div className="relative mx-auto max-w-7xl">
